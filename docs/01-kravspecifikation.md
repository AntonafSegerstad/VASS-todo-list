...
- **FR-7:** En användare ska kunna markera en todo som klar via en grön checkbox.
- **FR-8:** En klarmarkerad todo ska visuellt skilja sig från en omarkerad (t.ex. genomstruken text
  och/eller ifylld grön checkbox).
...
- **FR-19:** En todo utan angivet namn (eller med tomt namn) ska visas med ett standardvärde
  ("Okänd") istället för ett tomt fält.

## 4. GUI-krav

- **GUI-1:** Ytan som visar todos ska vara gul och post-it-liknande — detta är ett medvetet
...
| `completed` (eller motsvarande) | boolean | Klarstatus. |
| `name` | sträng | Valfritt namn/attribution kopplat till todon (se FR-18/FR-19). Visas som "Okänd" i GUI:t om tomt. |

**Öppen fråga till utvecklarteamet:** Exakt fältnamn för klarstatus (`completed`, `done`, `isDone`
etc.) och eventuella extra fält (t.ex. `createdAt`) är inte formellt låsta av detta dokument utan
ska verifieras mot den faktiska API-responsen innan tester skrivs mot exakta fältnamn — se
[`02-api-specifikation.md`](02-api-specifikation.md) för det som redan är bekräftat genom
observation.

**Öppen fråga till utvecklarteamet:** Exakt längdgräns och trimningsregler för `name`-fältet är
inte formellt låsta av detta dokument. GUI, `POST` och `PUT` är inte nödvändigtvis konsekventa med
varandra — verifiera faktiskt beteende innan tester skrivs mot exakta gränsvärden, se
[`02-api-specifikation.md`](02-api-specifikation.md).
